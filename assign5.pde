...
void initClocks() {
  ...
  for (int i = 0; i < cabbageX.length; i++) {
    ...
    for (int o = 0; o < cabbageX.length; o++) {
      while (clockX[i] == cabbageX[o] && clockY[i] == cabbageY[o]) {
        clockX[i] = SOIL_SIZE * floor(random(SOIL_COL_COUNT));
        clockY[i] = SOIL_SIZE * ( i * 4 + floor(random(4)));
      }
    }
...
void drawTimerUI() {
  ...
  timeTextColor = getTimeTextColor(gameTimer);
  fill(timeTextColor);
...
color getTimeTextColor(int frames) {				// Requirement #5
  if (frames>=120*60) {
    return #00ffff;
  } else if (frames>=60*60) {
    return #ffffff;
  } else if (frames>=30*60) {
    return #ffcc00;
  } else if (frames>=10*60) {
    return #ff6600;
  } else {
    return #ff0000;
  }
...
void drawCaution() {								// Requirement #6
  if(getEnemyIndexByRow(playerRow+5)!=-1){
    float cautionX = soldierX[getEnemyIndexByRow(playerRow+5)];
    float cautionY = soldierY[getEnemyIndexByRow(playerRow+5)]-SOIL_SIZE;
    image(caution,cautionX,cautionY,80,80);
  }
